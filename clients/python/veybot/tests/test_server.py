...
from veybot.dashboard import tail_jsonl
from veybot.db import Database, close_database, get_database, issue_key
from veybot.github_client import GitHubClient
from veybot.manual_triage import InvalidIssueRef, ManualTriageTimeout, await_terminal_state, parse_issue_ref
from veybot.sandbox import LocalGitTransport
...
def test_index_substitutes_replay_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("VEYBOT_REPLAY_TOKEN", "secret-token-7")
...
def test_api_status_reports_runtime_counts_and_inflight(settings: Settings) -> None:
    ...
    assert runtime["bot_login"] == "robveybot"
...
def test_api_logs_tails_jsonl_file(settings: Settings) -> None:
    log_path = settings.log_dir / "veybot.log.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    payloads = [
        {"ts": "2026-05-14T21:28:28Z", "level": "INFO", "logger": "veybot.queue", "msg": "dispatch loop online"},
        {
            "ts": "2026-05-14T21:28:54Z",
            "level": "INFO",
            "logger": "veybot.server",
            "msg": "skip",
            "event": "issues",
            "reason": "issues.labeled ignored",
        },
        {"ts": "2026-05-14T21:30:00Z", "level": "WARNING", "logger": "veybot.queue", "msg": "tool_end", "ok": False},
    ]
...
def test_parse_issue_ref_accepts_github_issue_urls() -> None:
    cases = (
        "https://github.com/santhreal/veyyon/issues/1348",
        "http://github.com/santhreal/veyyon/issues/1348",
        "github.com/santhreal/veyyon/issues/1348",
        "https://www.github.com/santhreal/veyyon/issues/1348",
        "https://github.com/santhreal/veyyon/issues/1348/",
        "https://github.com/santhreal/veyyon/issues/1348?foo=bar",
        "https://github.com/santhreal/veyyon/issues/1348#issuecomment-99",
        "  https://github.com/santhreal/veyyon/issues/1348  ",
    )
    for case in cases:
        assert parse_issue_ref(case) == ("santhreal/veyyon", 1348), case
...
def _enable_replay(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("VEYBOT_REPLAY_TOKEN", token)
    reset_settings_cache()
    return token
...
def test_trigger_triage_fetches_and_enqueues(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.post(
            ...
            headers={"X-Veybot-Replay-Token": token},
        )
    close_database()
...
def test_trigger_triage_rejects_repo_not_in_allowlist(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "VEYBOT_REPO_ALLOWLIST" in resp.json()["detail"]
...
def rate_limited_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> Settings:
    monkeypatch.setenv("VEYBOT_RATE_LIMIT_DEFAULT", "2")
...
def _allowlist(monkeypatch: pytest.MonkeyPatch, repos: str) -> None:
    monkeypatch.setenv("VEYBOT_REPO_ALLOWLIST", repos)
    reset_settings_cache()
...
def test_browse_returns_401_with_replay_enabled_without_valid_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        wrong = client.get(
            "/api/github/issues",
            headers={"X-Veybot-Replay-Token": f"{token}-wrong"},
        )
    close_database()
...
def test_webhook_directive_on_unknown_issue_is_queued_with_metadata(env) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert resp.status_code == 202
        # The synchronous response is the durable proof the handler ACCEPTED and
        # queued the directive (vs. skipping/rejecting it). Assert it here.
        assert resp.json()["state"] == "queued"
        row = get_database(cfg.sqlite_path).get_event("dir-1")
    close_database()
    assert row is not None
    # Do NOT assert `row.state == "queued"`: `create_app` starts the WorkerPool
    # dispatcher inside the TestClient lifespan, so the live loop can legitimately
    # claim this delivery (queued -> running) before this read — a ~1-in-3 flake.
    # The durable, race-free invariant is that an ACCEPTED event enters the
    # dispatch lifecycle and is never `skipped` (skipped is only stamped at
    # admission for rejected events).
    assert row.state != "skipped"
    directive = row.payload.get("_veybot_directive")
...
def test_webhook_directive_authorizes_deployed_app_login_without_author_association(
    ...
    monkeypatch.setenv("VEYBOT_BOT_LOGIN", "@veybot[bot]")
    monkeypatch.setenv("VEYBOT_REPO_ALLOWLIST", "can1357/widget")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    app = create_app(cfg)
...
def test_webhook_maintainer_bypasses_rate_limit(
    ...
    """Login in VEYBOT_MAINTAINER_LOGINS is always unlimited, even with NONE association."""
    monkeypatch.setenv("VEYBOT_MAINTAINER_LOGINS", "can1357")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        ...
        for i in range(4):
            resp = _post_issue_comment(
                ...
                user="can1357",
                number=300 + i,
                body=("@robveybot do X" if i == 3 else "comment"),
                association="NONE",
            )
            assert resp.status_code == 202
            states.append(resp.json()["state"])
    ...
    assert directive_event.payload.get("_veybot_directive") == {
        "body": "do X",
        "author": "can1357",
        "pragmas": [],
...
def stub_run_task(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    from veybot import tasks as tasks_module
...
async def test_handle_pr_conversation_skips_review_workspace_rows(
    ...
    from veybot import tasks
    from veybot.github_client import GitHubClient, PullRequestInfo
    ...
    payload = {
        ...
        "comment": {"user": {"login": "can1357"}, "body": "@robveybot please re-review", "id": 12},
        "repository": {"full_name": "octo/widget"},
        "_veybot_directive": {"body": "please re-review", "author": "can1357"},
    }
    await tasks.handle_pr_conversation(
        settings=settings,
        db=db,
...
async def test_review_pr_retries_when_ranked_but_not_submitted(
    ...
    from veybot import tasks
    from veybot.github_client import GitHubClient, IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_comment_directive_bootstraps_untriaged_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_veybot_directive": {"body": "please refactor X", "author": "can1357"},
    }
...
async def test_handle_comment_directive_reopens_finalized_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_veybot_directive": {"body": "redo the fix", "author": "can1357"},
    }
    await tasks.handle_comment(
        settings=settings,
        db=db,
...
async def test_directive_handler_attaches_thread_from_github(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_veybot_directive": {"body": "do X", "author": "can1357"},
    }
...
async def test_triage_issue_fails_open_when_timeline_fetch_errors(
    ...
    from veybot import tasks
    from veybot.github_client import GitHubError, IssueInfo, RepoInfo
