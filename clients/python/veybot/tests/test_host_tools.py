...
from veybot.github_client import GitHubClient, IssueInfo, RepoInfo
from veybot.host_tools import AbortController, ToolBindings, build
from veybot.sandbox import LocalGitTransport, Workspace
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    monkeypatch.setenv("VEYBOT_GH_PROXY_HMAC_KEY", "secret-proxy")
    ...
    assert env["VEYBOT_GH_PROXY_HMAC_KEY"] == ""
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bindings.workspace.root / ".veyyon-xdg" / "cache" / "bun-install")
    assert env["XDG_CACHE_HOME"] == str(bindings.workspace.root / ".veyyon-xdg" / "cache")
    assert env["TMPDIR"] == str(bindings.workspace.root / ".veyyon-tmp")
    ...
    assert (bindings.workspace.root / ".veyyon-tmp").is_dir()
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    assert kwargs["env"]["BUN_INSTALL_CACHE_DIR"].endswith("/.veyyon-xdg/cache/bun-install")
    ...
    def fake_run_repo_command(
        ...
        if command[:3] == ["git", "log", "--format=%H%x09%ae%x09%an"]:
            return subprocess.CompletedProcess(
                command,
                0,
                "abc123\trobveybot@example.invalid\trobveybot\n",
                "",
            )
...
def test_gh_post_comment_happy_path(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 999, "user": {"login": "robveybot"}, "body": "hi", "created_at": "t"})
...
def test_gh_post_comment_defaults_to_inbound_pr_thread(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        captured["url"] = str(request.url)
        return httpx.Response(201, json={"id": 7, "user": {"login": "robveybot"}, "body": "hi", "created_at": "t"})
...
def test_mark_unable_posts_comment_marks_needs_info_and_labels_issue(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 321, "user": {"login": "robveybot"}, "body": "x", "created_at": "t"})
...
def test_submit_pr_review_posts_summary_only_when_no_staged_comments(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"id": 45, "user": {"login": "robveybot"}, "body": "ok", "state": "COMMENTED", "submitted_at": "t"},
        )
...
def test_gh_push_branch_rejects_wrong_identity(db: Database, tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "robveybot <robveybot@example.invalid>" in msg
...
def test_gh_push_branch_rejects_invalid_identity_scan_range(db: Database, tmp_path: Path) -> None:
    ...
    for cmd in (
        ...
        [
            ...
            str(seed),
            "-c",
            "user.email=robveybot@example.invalid",
            "-c",
            "user.name=robveybot",
            "commit",
            "-m",
...
def test_gh_push_branch_runs_fix_and_check_before_pushing(
    ...
    assert lines[0] == "robveybot <robveybot@example.invalid> feat: follow-up", lines
    assert lines[1] == "robveybot <robveybot@example.invalid> init", lines
...
def test_gh_open_pr_runs_fix_then_check_and_amends_formatter_diff(
    ...
    lines = log.stdout.strip().splitlines()
    assert lines[0] == "robveybot|robveybot@example.invalid|feat: initial change"
...
def _question_handler(captured: dict[str, Any]):
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201,
            json={"id": 4242, "user": {"login": "robveybot"}, "body": "x", "created_at": "t"},
        )
...
def _new_ws_repo_with_merge(tmp_path: Path) -> "Workspace":
    """A real workspace repo whose `origin/main..HEAD` contains a fork/join.

    Topology (parent -> child):  base=origin/main -> P -> {X, Y} ; M=merge(X,Y)=HEAD.
    Commit dates are skewed so P (the fork point) carries a LATER date than its
    own children X/Y. Under the default `git rev-list --reverse` order (commit
    date, not topology) a child is then emitted before its parent, which is the
    exact condition the commit-message rewrite must be robust to. P's message
    carries a literal ``\n`` escape so the rewrite has real work to do.
    """
    import os
    import subprocess
    ...
    bare.mkdir()
    subprocess.run(
        ["git", "init", "--bare", "--initial-branch=main", str(bare)], check=True, capture_output=True
    )
    root = tmp_path / "ws"
    repo_dir = root / "repo"
    for p in (root, repo_dir, root / ".veyyon-session", root / "context" / "repro", root / "artifacts"):
        p.mkdir(parents=True, exist_ok=True)
    ...
    def git(*args: str, date: str | None = None) -> None:
        ...
        if date is not None:
            env["GIT_AUTHOR_DATE"] = env["GIT_COMMITTER_DATE"] = f"{date}T00:00:00"
        subprocess.run(["git", "-C", str(repo_dir), *args], check=True, capture_output=True, env=env)

    subprocess.run(["git", "init", "--initial-branch=main", str(repo_dir)], check=True, capture_output=True)
    git("config", "user.email", "robveybot@example.invalid")
    git("config", "user.name", "robveybot")
    git("commit", "--allow-empty", "-m", "base", date="2019-01-01")
    git("remote", "add", "origin", str(bare))
    git("push", "origin", "main")
    git("fetch", "-q", "origin")  # populate refs/remotes/origin/main
    branch = "farm/abc12345/merge-issue"
    git("checkout", "-q", "-b", branch)
    # P: fork point, broken message, LATE date so date-order pops it before X/Y.
    git("commit", "--allow-empty", "-m", r"P_subject\nP_body", date="2025-01-01")
    p_sha = subprocess.run(
        ["git", "-C", str(repo_dir), "rev-parse", "HEAD"], check=True, capture_output=True, text=True
    ).stdout.strip()
    git("checkout", "-q", "-b", "_bx", p_sha)
    git("commit", "--allow-empty", "-m", "X clean", date="2020-01-01")
    git("checkout", "-q", branch)
    git("commit", "--allow-empty", "-m", "Y clean", date="2021-01-01")
    git("merge", "-q", "--no-ff", "_bx", "-m", "M merge", date="2022-01-01")
...
def test_repair_commit_escapes_rewrites_fork_point_across_merge(db: Database, tmp_path: Path) -> None:
    """A broken `\\n` message on a fork commit is repaired even when a child of it
    sorts before it under default rev-list order.

    Regression for a silent partial-repair bug: the rewrite walked
    `origin/base..HEAD` in default `git rev-list --reverse` order, which is by
    commit DATE, not topology. With an in-range fork/join a child of the broken
    fork commit is emitted before it, so the child was rebuilt pointing at the
    OLD (unrepaired) parent while the repaired copy dangled off the other merge
    side — leaving the literal-`\\n` commit reachable from HEAD and pushed
    unrepaired, with no error (the detect->fix contract silently failing). The
    fix pins `--topo-order` so parents always precede children.
    """
    ws = _new_ws_repo_with_merge(tmp_path)
    ...
    try:
        ...
        host_tools._repair_commit_message_escapes(bindings, {}, tool_name="gh_push_branch")
    finally:
        _stop_loop(loop, thread)
    ...
    bodies = subprocess.run(
        ["git", "-C", str(ws.repo_dir), "log", "origin/main..HEAD", "--format=%B"],
        check=True,
        capture_output=True,
        text=True,
    ).stdout
    # No commit reachable from HEAD may still carry the literal escape.
