...
/// A path naming the calling process's descriptors or controlling terminal.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DescriptorPath {
	/// Descriptor under `/dev/fd`, `/proc/self/fd`, etc.
	Fd(ShellFd),
...
impl DescriptorPath {
	/// Parses an absolute descriptor path.
	...
	/// Error when opening fails due to missing descriptor or controlling terminal.
