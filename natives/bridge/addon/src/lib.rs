...
//! # Example
//! ```ignore
//! use veyyon_natives::text::visible_width;
//!
...
//! # Architecture
//! ```text
//! JS (natives/bridge/bindings) -> N-API -> Rust modules (clipboard/fd/glob/grep/html/highlight/sixel/text)
//! ```
...
fn configure_rayon_pool() {
	...
	let result = match plan {
		RayonPoolPlan::WorkerThreads(threads) => rayon::ThreadPoolBuilder::new()
			...
			.build_global(),
		RayonPoolPlan::SkipGlobalPool => {
			veyyon_uutils_ctx::set_rayon_global_pool_available(false);
			return;
		},
	};
	if result.is_ok() {
		veyyon_uutils_ctx::set_rayon_global_pool_available(true);
	}
}
...
#[napi(js_name = "__veyyonNativesV1_5_0")]
