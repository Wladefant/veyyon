...
// =========================================================================
// Pure helpers — re-exported for unit tests in `natives/bridge/bindings/test/`.
// =========================================================================
 ...
 * `__veyyonNativesV1_5_1`. `scripts/release.ts` bumps this name in lock-step
...
export function staleAddonMessage(stale, version) {
	...
	return (
		...
		"  Rebuild every variant for this version first: bun --cwd=natives/bridge/bindings run build"
	);
}
...
export function evaluateLoadedBindings(ctx, bindings, candidate) {
	...
	if (ctx.isWorkspaceLoad) {
		// Workspace dev (running out of `natives/bridge/bindings/native/` rather than a
		// `node_modules` install or compiled bundle): the local `.node` only gains
		// the renamed sentinel after `bun --cwd=natives/bridge/bindings run build`, so a
		...
		return {
			...
			message:
				...
				"Rebuild with: bun --cwd=natives/bridge/bindings run build\n" +
				`  (${candidate})\n`,
		};
	}
	return {
...
export function buildHelpMessage(ctx) {
	...
	return (
		"Provision it from the matching release: bun --cwd=natives/bridge/bindings run ensure\n" +
		"If developing locally, build with: bun --cwd=natives/bridge/bindings run build\n" +
