 ...
 * `natives/bridge/bindings/native/`. The single owner of source-install addon
...
export function formatProvisioningFailure(failures: AssetFailure[], cargoMissing: boolean): string {
	...
	return (
		...
		`${buildLine}\n` +
		"Fix: install Rust (https://rustup.rs) and run `bun --cwd=natives/bridge/bindings run build` in the checkout, " +
