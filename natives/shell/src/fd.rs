//! In-process `fd` builtin backed by `veyyon_walker`, `globset`, and `regex`.
...
fn search(
	...
	for search_path in &search_paths {
		...
		let outcome = match request.collect_with_heartbeat(cancel_heartbeat(cancelled)) {
			Ok(outcome) => outcome,
			Err(veyyon_walker::WalkError::Interrupted(_)) if cancelled.load(Ordering::Relaxed) => {
				break;
			},
...
fn matches_walker_type_filter(
	...
	let kind_matches = if filter.has_kind() {
		(filter.regular && file_type == veyyon_walker::FileType::File)
			|| (filter.directory && file_type == veyyon_walker::FileType::Dir)
			|| (filter.symlink && file_type == veyyon_walker::FileType::Symlink)
	} else {
		true
	};
	if !kind_matches {
...
/// closure, `veyyon_walker`'s per-entry heartbeat never checks the flag and a
...
fn walker_error_to_io(err: veyyon_walker::WalkError<io::Error>) -> io::Error {
	match err {
		veyyon_walker::WalkError::Interrupted(err) => err,
...
fn walker_collect_error_to_io(err: veyyon_walker::WalkError<String>) -> io::Error {
	match err {
		veyyon_walker::WalkError::Interrupted(err) => io::Error::other(err),
		veyyon_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
...
mod tests {
	...
	fn walk_request(tree: &std::path::Path) -> veyyon_walker::WalkRequest {
		veyyon_walker::WalkRequest::new(tree)
			.hidden(true)
			.gitignore(false)
			.emit_root(true)
	}
	...
	fn cancel_heartbeat_aborts_collect_with_heartbeat() {
		...
		assert!(
			matches!(err, veyyon_walker::WalkError::Interrupted(_)),
	...
	fn cancel_heartbeat_aborts_for_each_entry_with_heartbeat() {
		...
		let result = walk_request(&tree).for_each_entry_with_heartbeat(
			...
			|_entry| {
				...
				Ok::<_, std::io::Error>(veyyon_walker::WalkDecision::Include)
			},
			|_error| Ok::<_, std::io::Error>(veyyon_walker::WalkDecision::Include),
		);
