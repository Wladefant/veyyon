...
async fn create_session_for_run(
	...
	// VEYYON_DISABLE_UUTILS_BUILTINS; the destructive pair additionally honors
	// VEYYON_DISABLE_UUTILS_DESTRUCTIVE.
	if !uutils_env_disabled(config, "VEYYON_DISABLE_UUTILS_BUILTINS") {
		...
		if !uutils_env_disabled(config, "VEYYON_DISABLE_UUTILS_DESTRUCTIVE") {
			if !uutils_env_disabled(config, "VEYYON_DISABLE_RM_BUILTIN") {
				shell.register_builtin("rm", crate::coreutils::rm_builtin());
			}
			if !uutils_env_disabled(config, "VEYYON_DISABLE_MV_BUILTIN") {
	...
	// VEYYON_DISABLE_NOHUP_BUILTIN (session env or process env), in which case bare
	// `nohup` resolves to the real coreutils binary.
	let nohup_builtin_disabled = {
		let raw = config
			.session_env
			.as_ref()
			.and_then(|env| env.get("VEYYON_DISABLE_NOHUP_BUILTIN").cloned())
			.or_else(|| std::env::var("VEYYON_DISABLE_NOHUP_BUILTIN").ok());
...
async fn run_shell_command_single(
		...
		&& let Some(config) = options.minimizer.as_ref()
	{
		...
		if !buffered.exceeded {
			...
			// in `veyyon-natives`. A supported filter that runs but leaves the output
...
async fn run_shell_command_once(
	...
	let source_info = SourceInfo::from("veyyon-natives:command");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
async fn run_shell_command_streams(
	...
	ensure_trailing_newline_for_heredoc(&mut command);
	let source_info = SourceInfo::from("veyyon-shell:streams");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
/// backpressure: on a bounded channel (the veyyon-natives JS bridge) the send
...
impl builtins::Command for TimeoutCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let source_info = SourceInfo::from("veyyon-natives:timeout");
...
impl builtins::Command for NohupCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let mut params = context.params.clone();
			params.process_group_policy = ProcessGroupPolicy::NewProcessGroup;
			let source_info = SourceInfo::from("veyyon-natives:nohup");
			context
				.shell
				.run_string(command_line, &source_info, &params)
				.await
...
mod tests {
	...
	async fn uutils_builtins_respect_disable_env() {
		...
		let mut all_off = create_session(&session_with(&[("VEYYON_DISABLE_UUTILS_BUILTINS", "1")]))
			.await
			.expect("create_session");
		...
		let mut rm_off = create_session(&session_with(&[("VEYYON_DISABLE_RM_BUILTIN", "1")]))
			.await
			.expect("create_session");
	...
	///
	/// Lives in `veyyon-natives` because the brush-core crate is excluded from
	...
	fn unique_temp_dir(prefix: &str) -> std::path::PathBuf {
		...
		let nonce = std::time::SystemTime::now()
			...
			.expect("system time")
			.as_nanos();
		path.push(format!("veyyon-shell-{prefix}-{}-{nonce}", std::process::id()));
...
"#,
	...
	/// "veyyon-natives:command: syntax error at end of input". The segmented
	...
	async fn embedded_external_command_runs_in_its_own_session() {
		...
		// Build the same kind of session veyyon-natives uses in production.
		let config = ShellConfig { session_env: None, snapshot_path: None, minimizer: None };
		let mut session = create_session(&config).await.expect("create_session");
	...
	/// leave the embedded host (Veyyon) stopped on its next tty read. The
