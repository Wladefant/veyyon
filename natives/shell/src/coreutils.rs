...
//!
//! Each builtin installs a [`veyyon_uutils_ctx`] scope — the command's stdio
...
fn run_caught(run: UutilRun, argv: Vec<OsString>) -> i32 {
	...
	if let Ok(code) = catch_unwind(|| run(argv)) {
		...
		let _ = writeln!(veyyon_uutils_ctx::stderr(), "{name}: internal error");
		1
	}
}
...
uutil_builtin!(pub fn grep_builtin => veyyon_uu_grep::run);
uutil_builtin!(pub fn rg_builtin => veyyon_uu_grep::run_rg);
...
uutil_builtin!(pub fn diff_builtin => veyyon_uu_diff::run);
...
mod tests {
	...
	fn scope_io(stderr: Box<dyn Write + Send>) -> veyyon_uutils_ctx::ScopeIo {
		veyyon_uutils_ctx::ScopeIo {
	...
	fn run_in_scope(run: UutilRun, argv: Vec<OsString>) -> (i32, String) {
		...
		let code =
			veyyon_uutils_ctx::scope(scope_io(Box::new(ChanWriter(tx))), || run_caught(run, argv));
		let mut err = Vec::new();
