...
//!
//! # Overview
//! `veyyon-walker` owns the native directory-read fast path that higher-level
...
impl DirScratch {
	...
	#[expect(
		clippy::unused_self,
		reason = "the unix variant reads self.name_bytes; the windows variant reads the entry \
		          alone, and the signatures must match"
	)]
