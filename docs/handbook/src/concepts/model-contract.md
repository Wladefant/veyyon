...
Subagent models are not roles. They are configured in the Subagents settings area, on two exclusive scopes.
With **Same Model for All Subagents** off, the first of these sets the model: that agent's row in
`subagent.agents`, then the agent definition's own `model:`, otherwise the `default` model role.
With it on, `subagent.model` sets it for every agent and the rows above are not read. There is no
