...
| `CMD_API_KEY`                    | Command Code auth                                | Using `command-code` provider                                  | Preferred key alias; endpoint `https://api.commandcode.ai/provider/v1`, default `claude-sonnet-4-6`                |
...
| `COMMANDCODE_API_KEY`            | Command Code auth                                | Using `command-code` provider without the two aliases above    | Last fallback; the spelling the upstream CLI reads                                                                        |
...
- Every eval kernel spells its IPC trace variable `VEYYON_<LANGUAGE>_IPC_TRACE`, with the language spelled out
...
| `VEYYON_EVAL_SYSTEM_PROMPT_STATEMENTS` | The statement-level counterpart to the section override, same benchmark-only status. See [system prompt architecture](../../../internal/system-prompt-architecture.md#the-two-escape-hatches-and-why-they-are-env-vars)                                                                                                                                                                                                                     |
