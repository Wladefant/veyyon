...
- A pseudoconsole host (ConPTY's conhost) takes the in-place exclusion for the opposite reason: it owns the grid, reprinting its whole viewport from `CSI H` on every resize and re-homing the cursor, so a settled resize has neither a recoverable anchor nor a grid nobody else repaints. `Terminal.hostOwnsGridOnResize` — answered by `ProcessTerminal` from its construction-time `conpty` override, so the same value gates the kitty-flag choice and large-write chunking — keeps the borrow there, whose settled transaction ends in the scrollback rebuild that erases the host's copy.
