...
- Key collaborators:
  ...
  - `packages/coding-agent/src/internal-urls/agent-protocol.ts`: resolve `agent://<id>` to saved agent output.
  ...
  - `packages/coding-agent/src/sdk.ts`: child-session router/tool wiring and per-agent `AgentOutputManager`.
  - `docs/internal/task-agent-discovery.md`: deeper discovery and precedence notes.

## Inputs

The wire schema is shape-swapped by `agent.batch` (default on). One unit of work is the task item `{ name?, agent?, task, cwd?, isolated? }` (`isolated` only when `agent.isolation.mode` is not `none`):

- **Batch shape** (`agent.batch` on): `{ context, tasks: item[] }`: one agent per item, all run under the same fan-out rules; there is no top-level agent field. `context` is **required** shared background rendered into every spawned agent's system prompt (`CONTEXT` section); `agent` and `isolated` are per item, so one call may mix agent types.
- **Flat shape** (`agent.batch` off): `{ ...item }`: exactly one spawn per call. Shared background goes into a `local://` file (e.g. `local://ctx.md`) that each spawn's `task` references; agents share the parent's `local://` root.

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `context` | `string` | Yes (batch) | Shared background prepended to every spawn of the call via the agent system prompt. Rejected when `agent.batch` is off. |
| `tasks` | `array` | Yes (batch) | One task item per agent. Provided names must be unique within the call (case-insensitive). Rejected when `agent.batch` is off. |
...
| `isolated` | `boolean` | No | Run in an isolated workspace and return patches. Exists only when `agent.isolation.mode` is not `none`; per item in batch shape, top-level in flat shape. Isolated agents are torn down at completion, not revivable. |
...
Runtime stays permissive: the flat form is accepted even while `agent.batch` is on (internal callers such as the commit flow's `analyze_files`, and stale transcripts). The model only ever sees one shape.
...
- Every agent with an artifacts dir writes `<id>.md`; `agent://<id>` resolves to that file.
- If the output file is JSON, `agent://<id>/<path>` and `agent://<id>?q=<query>` perform JSON extraction.
- Each agent gets `<id>.jsonl` session history when the parent persists artifacts; `history://<id>` renders it as a concise transcript (works for live and parked agents).
...
2. `execute(...)` repairs raw params (`repairTaskParams`), then validates: `schema` is always rejected; `tasks`/`context` are rejected unless `agent.batch` is on; batch calls need a non-empty `tasks` (a `task` per item, unique provided names), a non-empty shared `context`, and no top-level `task` alongside `tasks`; flat calls need `task`. The call is then normalized into its spawn list (`resolveSpawnItems`).
...
4. Background execution (any non-blocking item with `async.enabled=true` and an `AsyncJobManager`):
   ...
   - each job body acquires the session-scoped `Semaphore` (one per `TaskTool` instance, sized from `agent.maxConcurrency` at first use), marks the job running, runs `#executeSync(...)` with that spawn's params, and reports progress through the shared `buildAsyncDetails`/`onUpdate`;
...
9. If `isolated`, it requires a git repo (`getRepoRoot(...)` / `captureBaseline(...)`), maps `agent.isolation.mode` to a backend-kind hint (`parseIsolationMode`), and materializes the workspace via the natives PAL (`ensureIsolation` → `isoResolve`/`isoStart`), walking the candidate list when a backend is unavailable.
10. Artifacts dir comes from the parent session file when available, otherwise a temp dir. When the session is executing an approved plan, the plan reference is handed to the agent.
...
12. `runSubprocess(...)` creates a child agent session with an isolated settings snapshot (forcing `async.enabled = false` and `bash.autoBackground.enabled = false`: agents are internally synchronous), child `agentId` equal to the allocated id, child internal URL router/`AgentOutputManager`, output schema, the shared `context` (batch calls) in the system prompt's `CONTEXT` section, and the IRC peer roster in the system prompt.
13. Child tool availability: explicit `agent.tools` if provided; auto-add `task` when the agent has `spawns` and depth allows; strip `task` past `agent.maxNestedSpawnDepth`; ensure `irc` is present in explicit tool lists; expand `exec` to `eval` + `bash`; strip parent-owned `todo`.
...
16. Lifecycle thereafter: `idle` agents are parked after `agent.idleTtlMs`, which defaults to 5 minutes regardless of model or provider. Parking disposes the live session but retains the `AgentRef` and session file; messaging (`irc`), or opening the agent in the agent dashboard, revives it back to `idle`. `"Main"` is never parked.
...
- Batch mode (`agent.batch`, default on)
...
- Isolation mode (`agent.isolation.mode`): `none`, `auto`, `apfs`, `btrfs`, `zfs`, `reflink`, `overlayfs`, `projfs`, `block-clone`, `rcopy` (legacy `worktree`, `fuse-overlay`, `fuse-projfs` accepted for back-compat); the PAL resolves the actual backend with fallback.
...
- Which of those the model may pick is `agent.agents` (the Agents settings tab, or `/agents`). Only the general-purpose `deep` worker and agents you wrote yourself are offered by default; the bundled specialists are not, so their descriptions cost nothing until you enable one. There are two states and no third: a disabled agent is refused even when a caller names it outright. The one exception is a `/` command that declares the agent for its own turn (`CustomCommand.spawnsAgents` → `session.agentGrantedThisTurn`), which is how `/review` reaches `reviewer` without `reviewer` being enabled.
- The model each spawn runs comes from `resolveAgentModel` (`packages/coding-agent/src/task/agent-settings.ts`), on two exclusive scopes selected by `agent.sharedModel`. On, `agent.model` decides for every agent and nothing else is read. Off, highest layer first: the `agent.agents.<name>` lane governing the spawn's own depth, then the agent definition's `model:`, otherwise the `default` model role. `agent.modelByDepth` is retired in both. A configured value that matches no available model refuses the spawn and names the setting, rather than falling through to the next layer.
...
## Limits & Caps
- Concurrency: one session-scoped `Semaphore` sized from `agent.maxConcurrency` at first use (later setting changes do not resize it) bounds concurrent agents across parallel `task` calls: both async job bodies and the sync fallback acquire it.
...
- Parameter validation failures are returned as normal tool text with empty `results`:
  ...
  - `tasks` / `context` while `agent.batch` is disabled
...
- `irc` availability is derived, not configured (`isIrcEnabled` in `packages/coding-agent/src/tools/agent/irc.ts`): it exists exactly when there is someone to message: the session can spawn agents, or it is an agent itself. Messaging is the only follow-up path to a finished agent, so task without irc would strand idle agents.
- `tools.discoveryMode: all` hides non-essential built-ins, and `task` is a carve-out. At `agent.delegation` `preferred` (the default) or `required` the prompt asks you to delegate, so `task` stays in the request; hiding it would leave an instruction you cannot follow. At `allowed` nothing asks for delegation, so `task` is hidden like any other discoverable built-in and you reach it through `search_tool_bm25`, and the prompt drops its delegation section to match. The decision is made in `sdk.ts` before agent discovery runs, from the strength alone: whether any agent is enabled is not knowable yet at that point.
- Only the general-purpose `deep` agent ships enabled; other agent names require `agent.agents.<name>.enabled = true` before the model may choose them, including definitions added under `~/.veyyon/subagents/`.
- Agents are internally synchronous: the executor forces `async.enabled = false` and `bash.autoBackground.enabled = false` in the child settings snapshot, so there are no fire-and-forget grandchildren.
- Agent discovery precedence is first-wins by exact name: the authored `~/.veyyon/subagents` dir first (a repository's `.veyyon/agents` is not a source, and `.claude`/`.codex`/`.gemini` agent dirs are skipped), then extension-package agent dirs, then Claude plugin agent dirs, bundled agents last. Create-time discovery is memoized per cwd for the prompt description; execution-time discovery stays fresh.
