...
- `stripAnsi` removes the escape families it did not know: the string sequences (`ESC P` DCS, `ESC X` SOS, `ESC ^` PM, `ESC _` APC), nF (`ESC ( B`, the ASCII charset select every editor sends on exit), and the two-byte short class (`ESC 7`/`ESC 8` cursor save and restore, `ESC =`/`ESC >` keypad mode, `ESC c` reset). It knew CSI and OSC, and everything else fell to the stray-escape rule, which drops the escape byte and keeps what follows as text — so a captured progress bar read `7done8`, a reset line began `(B`, and a kitty graphic or a sixel image published its entire payload into the text a model reads. The short class is `0x30-0x7e` minus the six introducers, which is what keeps the new arm from eating a CSI's parameters. `ESC b` is EMI, an Fs control, so `a\x1bb` now strips to `a` rather than `ab`: the shared corpus case that expected the letter recorded the missing grammar, not a decision, and the width engine in `crates/veyyon-text` has always charged zero columns for it.
- `eval-prompt-overrides` rejects a non-object `VEYYON_EVAL_PROMPTS` payload through `isRecord` rather than an inline copy of the same three-clause predicate, which `type-guards` owns. No behaviour change: the accepted payloads and the refusal text are the same.
- `fetchWithRetry` rejects with a cancellation `isAbortError` recognises on all three of its abort paths, instead of a bare `Error` whose only claim to being one was the word `aborted` in its message.
...
- Comment prose that quoted a chat message was deleted from `fault-sink`. Comments only, so nothing this package exports behaves differently.

### Fixed

- `extractRetryHint` reads a retry window out of an error message's prose (`retry-after-ms=62000`, `retry-after: 60`) and out of Anthropic's per-bucket reset headers. The `-ms` spelling is the one this codebase's own formatter appends to a provider error, and a caller holding only text (the auth gateway passes the message with no headers left to read) could not see the hint veyyon had just written for it, so it fell back to a flat default block and returned an exhausted account to the pool early.
- `fetch-retry`: a 429 that states its own retry window in prose or in Anthropic's per-bucket reset headers is now honoured instead of being handed to exponential backoff. `extractRetryHint` read `retry-after` and `x-ratelimit-reset-after` and nothing else, so `429 Rate limit exceeded. Please try again later. retry-after 60` in a message body yielded no hint, and an Anthropic 429 that omits `retry-after` (a meaningful share of them do) yielded no hint either even though the response says per bucket when the limit refills. Backoff then retried inside the stated window and was rejected again, which is the shape of the 85 rate-limit errors that repeated immediately. `ANTHROPIC_RESET_HEADERS` is the reset/remaining pairs the provider sends and `anthropicResetDelayMs` reduces them to one delay: when a bucket reports `remaining: 0`, or the unified status is `rejected`, it takes the LONGEST such reset, because retrying while the rejecting bucket is empty always fails; with no exhaustion evidence it takes the SHORTEST, because guessing long strands the caller behind a window that may not apply. Any derived window is capped at `ANTHROPIC_RESET_MAX_HOLD_MS` (24h), and a past or unparseable clock is treated as absent rather than as zero. `retry-after` still outranks the clocks, which are consulted only when the provider gave no direct answer.
...
- `fetch-retry`: `fetchWithRetry` stops re-sending a request whose transport error names an HTTP/2 code this module has already ruled deterministic. The classifier and the retry loop live in the same file and disagreed: `http2RetryVerdict` says `CANCEL`, `FLOW_CONTROL_ERROR`, `FRAME_SIZE_ERROR`, `COMPRESSION_ERROR`, `INADEQUATE_SECURITY` and `HTTP_1_1_REQUIRED` fail the same way on every replay, while the loop's network-error branch consulted no classifier at all and spent the whole attempt budget on them. Each of those attempts re-sends the entire request body, so a provider call carrying a large prompt paid for four extra copies of it to reach the identical answer. `CANCEL` is the one that bites: it is usually our own abort arriving through a per-attempt signal the loop cannot see on `signal`. Retryable and unrecognised codes keep the budget they had.
