...
- A bridge-routed Codex turn now also carries the trusted Codex environment context the local `codex-chatgpt-web` daemon requires, which is what lets a Full-mode daemon complete a turn at all. A Full-mode daemon resolves that envelope for EVERY turn it takes — text-only turns and compaction requests included, before the execution key and before any browser work — and refused every Veyyon turn with "ChatGPT web turn is missing cwd in trusted Codex environment context"; the previous entry's claim that bridge turns were "no longer refused before any browser work" was therefore true only against a browser-only daemon. The transport now sends one `<environment_context>` user item, stamped with the same turn id and placed immediately before the current-turn user item, which is the only one of the daemon's acceptance paths this transport can reach (the others need a server-owned item `id` the Codex request transformer strips by design). It carries the caller's own session working directory and nothing else: the single workspace root is that directory, and the sandbox is declared as `permission_profile type="disabled"` with `file_system type="unrestricted"` — the daemon's `dangerFullAccess`, which states that no sandbox is in force. That is the truthful declaration for Veyyon, whose tools run as the host user with no filesystem boundary; the daemon's other two policies would assert containment that does not exist. When the caller supplies no cwd, or a relative one, nothing is invented and nothing is resolved against the running process: the envelope is omitted, a warning names the missing prerequisite, and the daemon's own refusal stands. Both clauses of the existing gate still apply, so OpenAI's host and every other Codex-compatible server on loopback are sent byte-identical bodies with no envelope and no stamp. Unchanged by this: no authenticated ChatGPT browser profile exists on this machine, so no real account round trip — models, turn, cancellation or tool call — has been exercised.
