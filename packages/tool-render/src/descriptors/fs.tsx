...
function EditBody({ args, result }: ToolRenderProps): ReactNode {
	const input = str(args.input) ?? str(args._input);
	const details = detailsRecord(result);
...
function SetCwdSummary(props: ToolRenderProps): ReactNode {
...
function SetCwdBody(props: ToolRenderProps): ReactNode {
	const { result } = props;
	const change = readChange(props);
	const moved = change.previous !== null && change.cwd !== null && change.previous !== change.cwd;
	return (
		<>
			...
			<KvGrid>
				{change.previous && (
					<Kv k="from">
						<PathText path={change.previous} />
					</Kv>
				)}
				{change.cwd && (
					<Kv k="to">
						<PathText path={change.cwd} />
					</Kv>
				)}
...
export const fsDescriptors: readonly ToolDescriptor[] = [
	{ name: "read", aliases: ["read_file"], Summary: ReadSummary, Body: ReadBody },
	{ name: "write", aliases: ["write_file"], Summary: WriteSummary, Body: WriteBody },
	{ name: "edit", aliases: ["apply_patch"], Summary: EditSummary, Body: EditBody },
	{ name: "set_cwd", Summary: SetCwdSummary, Body: SetCwdBody },
];
