...
function createAgentRunMonitor(args: RunMonitorArgs): AgentRunMonitor {
	const {
		index,
		id,
		agent,
	...
	const processEvent = (event: AgentEvent) => {
		...
		switch (event.type) {
			...
			case "tool_execution_end": {
				...
				// Check for registered agent tool handler
			...
			case "tool_execution_update": {
				// Surface nested-agent progress mid-flight. The child task
			...
			case "message_end": {
				...
				if (role === "assistant") {
					progress.requests += 1;
					const messageContent = messageField(event.message, "content") || messageField(event, "content");
				...
				const messageUsage = messageField(event.message, "usage") || messageField(event, "usage");
			...
			case "agent_end":
				...
				if (event.messages && Array.isArray(event.messages)) {
					for (const msg of event.messages) {
						...
						const messageContent = messageField(msg, "content");
						if (messageContent && Array.isArray(messageContent)) {
							for (const block of messageContent) {
