...
/**
 * Pluggable operations for the file-pattern search engine.
	...
	return untilAborted(signal, async () => {
		...
		for (const rawPattern of aliasResolvedPatterns) {
			...
			if (isSshUrl(rawPattern)) {
				throw new ToolError(
					`File search cannot operate on a remote ssh:// path: ${rawPattern}. ssh:// has no local file to scan; use \`read ${rawPattern}\` to list or inspect the remote path.`,
				);
			}
			if (hasGlobPathChars(rawPattern)) {
				throw new ToolError(`File patterns are not supported for internal URLs: ${rawPattern}`);
			}
