 ...
 *
 * A backgrounded `task` returns a result while its agent keeps running, with
 ...
 * switch. Any of those sealed a running agent's card mid-flight and dropped
...
describe("a running agent survives a transcript rebuild", () => {
	...
	beforeEach(async () => {
		...
		tempDir = TempDir.createSync("@pi-running-agent-rebuild-");
