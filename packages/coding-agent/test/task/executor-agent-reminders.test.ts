...
describe("runSubprocess yield reminders", () => {
	...
	it("waits for session_start extension user messages before prompting the agent", async () => {
		...
		const result = await runSubprocess({
			...baseOptions,
			id: "agent-session-start-extension",
		});
	...
	it("skips modelRegistry.refresh when reusing the parent registry", async () => {
		...
		await runSubprocess({ ...baseOptions, id: "agent-skip-refresh", modelRegistry });

		expect(refreshSpy).not.toHaveBeenCalled();
		expect(createAgentSessionSpy).toHaveBeenCalledTimes(1);
	});

	it("splices the agent role prompt before the trailing system section", async () => {
		let userPrompt = "";
		...
		expect(systemPrompt?.[2]).not.toContain("agent-context-system");
		// The parent-conversation CONTEXT section is gone: agents get their
	...
	it("sends reminder prompt when agent stops without yield", async () => {
	...
	it("keeps null yield warning when agent submits success without data", async () => {
		...
		const result = await runSubprocess({ ...baseOptions, id: "agent-2" });
		expect(result.output).toContain("SYSTEM WARNING: Agent called yield with null data.");
	});
	...
	it("retries when yield tool returns an error before succeeding", async () => {
		...
		const result = await runSubprocess({ ...baseOptions, id: "agent-err-then-success" });
		expect(prompts).toHaveLength(2);
		expect(result.exitCode).toBe(0);
		expect(result.output).toContain('"ok": true');
	});

	it("waits for yield-triggered abort cleanup before resolving the agent", async () => {
		...
		let settled = false;
		const resultPromise = runSubprocess({ ...baseOptions, id: "agent-yield-abort-cleanup" }).finally(() => {
			settled = true;
		});
	...
	it("marks pre-aborted subprocess with a concrete reason", async () => {
		...
		const result = await runSubprocess({
			...baseOptions,
			id: "agent-cancelled-before-start",
	...
	it("surfaces the assistant abort message instead of 'Cancelled by caller' on an internal turn abort", async () => {
		// No caller signal and no runtime limit: the agent's own turn ended with
		...
		const result = await runSubprocess({ ...baseOptions, id: "agent-internal-abort" });
	 ...
	 *
	 * Also pins that the reminder ladder does NOT run: re-prompting an agent whose provider just
	 * refused would hit the same wall three more times and multiply the failure noise.
	 */
	it("fails with the provider's message when the agent's turn ended in an error", async () => {
		...
		const result = await runSubprocess({ ...baseOptions, id: "agent-model-error" });

		expect(result.exitCode).toBe(1);
		expect(result.aborted).toBe(false);
		expect(result.abortReason).toBeUndefined();
		expect(result.stderr).toBe("rate limit reached for this model");
		// One prompt: the task. No reminders were sent to an agent whose provider had just refused.
	...
	it("uses modelRegistry.authStorage when only options.modelRegistry is provided", async () => {
		...
		await runSubprocess({ ...baseOptions, id: "agent-registry-only", modelRegistry });
	...
	it("fails after one reviewer repair attempt and preserves accepted findings", async () => {
		...
		const result = await runSubprocess({
			...baseOptions,
			id: "agent-review-schema-repair-exhausted",
			agent: { ...baseAgent, output: reviewerSchema },
			outputSchema: reviewerSchema,
		});
	...
	it("logs reminder-loop aborts at debug, not error (issue #1623)", async () => {
		...
		const result = await runSubprocess({
			...baseOptions,
			id: "agent-abort-during-reminder",
			signal: abortController.signal,
		});

		expect(result.aborted).toBe(true);
		expect(errorSpy).not.toHaveBeenCalledWith("Agent prompt failed", expect.anything());
		expect(debugSpy).toHaveBeenCalledWith("Agent prompt aborted");
	});
});
...
describe("runSubprocess telemetry propagation", () => {
	...
	it("derives agent telemetry from parent: keeps tracer/hooks, swaps agent identity, clears conversationId", async () => {
		...
		await runSubprocess({ ...baseOptions, id: "agent-telemetry-derive", parentTelemetry });
	...
	it("forwards no telemetry when the parent has none", async () => {
		...
		await runSubprocess({ ...baseOptions, id: "agent-telemetry-none" });
